...
cc_smooth %>%
  ggplot(aes(word_num, score_smooth))          +
    ...
    geom_text(x=2892, y=0, label=cc_trimmed_text[353]) +
    geom_text(
      data=word_labels,
      aes(x=x, y=-1.5*ymax, label=stave_text), vjust=1, hjust=0) -> g
g
png(filename="../data/cc_sentiment.png", width=640, height=960)
g
dev.off()
```
